...
- estimated_cost must be null unless the amount is explicitly present in RAW_TEXT or user-provided numbers.
...
- what_not_to_say MUST be short phrases only (2–8 words). No sentences.`;
...
function looksLikeHouseThreat(s) {
  if (!s) return false;
  ...
  return (
    ...
    t.includes("our team will visit")
  );
}

function toShortPhrases(arr) {
  // Enforce short phrases only; convert long advisory sentences into compact phrases.
  const out = [];
  const push = (s) => {
    const v = String(s || "").trim();
    if (!v) return;
    // If it's long, compress to first clause / first 8 words
...
function normalizeOutput(obj, salary, essentials) {
  ...
  safe.calm_summary = {
    ...
    plain_english: String(obj?.calm_summary?.plain_english ?? "").slice(0, 1200),
    what_to_do_first: String(obj?.calm_summary?.what_to_do_first ?? "").slice(0, 700),
    why_this_matters: String(obj?.calm_summary?.why_this_matters ?? "").slice(0, 700),
  };
  ...
  const normMsgs = cms
    .map((m) => ({
      ...
      risk_tag:
        m?.risk_tag === "info" || m?.risk_tag === "pressure" || m?.risk_tag === "intimidation" || m?.risk_tag === "legal_claim"
          ? m.risk_tag
          : "pressure",
      note: String(m?.note ?? ""),
    }))
  ...
  safe.extracted = {
    salary: sal,
    essentials: ess,
    disposable_income: toNumberOrNull(disp),
    debts: normDebts,
  ...
  const hasThreatMsg = normMsgs.some((m) => looksLikeHouseThreat(m.text));
  const hasThreatDetected = normDetected.some((x) => x.classification === "house_visit_threat");
  if (hasThreatMsg && !hasThreatDetected) {
    normDetected.push({
      text: normMsgs.find((m) => looksLikeHouseThreat(m.text))?.text || "house visit mentioned",
  ...
  const pressure =
    ra?.collection_pressure_level === "low" ||
    ra?.collection_pressure_level === "medium" ||
    ra?.collection_pressure_level === "high" ||
    ra?.collection_pressure_level === "severe"
  ...
  const normPlan = plan
    .map((p, idx) => ({
      day: toNumberOrNull(p?.day) ?? idx + 1,
  ...
  // Enforce Day 1 = pay minimum if possible
  ...
  if (firstMinDebt && disposable !== null && disposable >= (firstMinDebt.minimum_due ?? 0)) {
    ...
    const rest = normPlan.filter((x) => x.day !== 1);
    safe.next_7_days_plan = [day1, ...rest].slice(0, 7);
  } else {
    safe.next_7_days_plan = normPlan.slice(0, 7);
  }
  ...
  const normDefault = {
    ...
    what_not_to_say: toShortPhrases(def?.what_not_to_say),
  ...
  if (normDefault.what_to_say.length === 0) {
    normDefault.what_to_say = [
      ...
      "I can make a payment now and I’m requesting options for a payment plan or extra time.",
      "Please confirm the current outstanding amount and the official channel for payment.",
    ];
  }
  if (normDefault.what_not_to_say.length === 0) {
    normDefault.what_not_to_say = ["I cannot pay", "I will default", "I refuse"];
  }
  ...
  for (const d of normDebts) {
    ...
    if (existing && typeof existing === "object") {
      scriptsOut[k] = {
        ...
        what_not_to_say: toShortPhrases(existing?.what_not_to_say).length ? toShortPhrases(existing?.what_not_to_say) : normDefault.what_not_to_say,
      ...
      if (d.minimum_due !== null) lines.push("I am making the minimum payment now and want confirmation once it reflects.");
      lines.push("Please share options for a simple payment plan or short extension.");
      scriptsOut[k] = {
  ...
  // Fill calm_summary defaults if empty
  if (!safe.calm_summary.plain_english) {
    safe.calm_summary.plain_english =
      "I can see overdue payments and a pressure message. The good news is you can stabilize this with one small step first, then a calm plan over the next few days.";
  }
